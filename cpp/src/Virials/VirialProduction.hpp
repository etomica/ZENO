...
VirialProduction(IntegratorMSMC<T, RandomNumberGenerator> & rIntegrator,
                 ...
                 refMeter(MeterOverlap<T, RandomNumberGenerator>(refClusterRef, refClusterTarget, alpha, 0, 1)),
                 targetMeter(MeterOverlap<T, RandomNumberGenerator>(targetClusterTarget, targetClusterRef, 1/alpha, 0, 1)),
...
analyze() {
    ...
    if (std::isnan(targetStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR])) {
        idealTargetFraction = 1;
        return;
    }
    if (std::isnan(refStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR])) {
        idealTargetFraction = 0;
        return;
    }
    int numTargets = targetMeter.getNumData();
    double alpha = refMeter.getAlpha()[0];
    alphaStats[0] = refStats[1][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG]/targetStats[numTargets-1][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG]*alpha;
    alphaStats[1] = alpha*MeterOverlap<T, RandomNumberGenerator>::ratioErr(refStats[1][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], refStats[1][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR],
                                                 targetStats[numTargets-1][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], targetStats[numTargets-1][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], 0);
    ...
    double vd = refRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG];
    double ed = refRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR];
    for (int i=0; i<numTargets-1; i++) {
        fullStats[i][0] = refIntegral * targetRatioStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG] / vd * alpha;
        fullStats[i][1] = fabs(refIntegral) * MeterOverlap<T, RandomNumberGenerator>::ratioErr(targetRatioStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], targetRatioStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], vd, ed,0) * alpha;
        double vi = targetRatioStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG];
        double ei = targetRatioStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR];
        for (int j=0; j<numTargets-1; j++) {
            if (j==i) {
                fullBCStats[i][i] = 1;
                continue;
            }
            double vj = targetRatioStats[j][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG];
            double ej = targetRatioStats[j][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR];
            fullBCStats[i][j] = fullBCStats[j][i] = MeterOverlap<T, RandomNumberGenerator>::ratioCor(vi, vj, vd, ei, ej, ed, cij[i][j], 0, 0);
        }
    }

    double oldFrac = ((double)targetSteps)/(targetSteps + refSteps);
    double refErrorRatio = refRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR]/fabs(refRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG]);
    if (std::isnan(refErrorRatio) || refErrorRatio > 1) refErrorRatio = 1;
    double targetErrorRatio = targetRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR]/fabs(targetRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG]);
...
printResults(const char **targetNames) {
    ...
    printf("reference ratio:           % 22.15e  error: %12.5e   cor: % 7.5f\n", refRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], refRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], refBCStats[0][1]);
    printf("reference average:         % 22.15e  error: %12.5e  acor: % 7.5f\n", refStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], refStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], refStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ACOR]);
    printf("reference overlap average: % 22.15e  error: %12.5e  acor: % 7.5f\n", refStats[1][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], refStats[1][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], refStats[1][MeterOverlap<T, RandomNumberGenerator>::AVG_ACOR]);
    printf("target ratio:              % 22.15e  error: %12.5e   cor: % 7.5f\n", targetRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], targetRatioStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], targetBCStats[0][numTargets-1]);
    printf("target average:            % 22.15e  error: %12.5e  acor: % 7.5f\n", targetStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], targetStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], targetStats[0][MeterOverlap<T, RandomNumberGenerator>::AVG_ACOR]);
    printf("target overlap average:    % 22.15e  error: %12.5e  acor: % 7.5f\n", targetStats[numTargets-1][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], targetStats[numTargets-1][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], targetStats[numTargets-1][MeterOverlap<T, RandomNumberGenerator>::AVG_ACOR]);
    ...
    for (int i=1; i<numTargets-1; i++) {
        ...
        printf("%-26s % 22.15e  error: %12.5e  acor: % 7.5f\n", name, targetStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], targetStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], targetStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_ACOR]);
        ...
        printf("%-26s % 22.15e  error: %12.5e   cor: % 7.5f\n", name, targetRatioStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_AVG], targetRatioStats[i][MeterOverlap<T, RandomNumberGenerator>::AVG_ERR], targetBCStats[i][numTargets-1]);
