...
analyze(double &jBest) {
    ...
    for (int j=0; j<numAlpha; j++) {
        lnRatio[j] = std::log(refOverStats[j][MeterOverlap<T>::AVG_AVG]/targetOverStats[j][MeterOverlap<T>::AVG_AVG]);
        if (j>0 && lnRatio[j]*lnRatio[j-1] <= 0) {
            // linear interpolation on log scale
            double xj = lnRatio[j-1]/(lnRatio[j-1]-lnRatio[j]);
            jBest = j-1 + xj;
            newAlpha = std::exp(std::log(alpha[j-1]) + xj*(std::log(alpha[j]/alpha[j-1])));

            double ratio1 = std::exp(lnRatio[j-1]);
            ...
            double ratio2 = std::exp(lnRatio[j]);
    ...
    int jb = (std::abs(lnRatio[0]) < std::abs(lnRatio[numAlpha-1])) ? 0 : numAlpha-1;
    jBest = jb;
    newAlpha = alpha[jb];
