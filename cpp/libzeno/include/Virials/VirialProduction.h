...
VirialProduction(IntegratorMSMC<T, RandomNumberGenerator> & rIntegrator,
                 ...
                 targetMeter(MeterOverlap<T>(targetClusterRef, 1/alpha, 0, 1, targetClusterTarget.numValues())),
...
analyze() {
    ...
    for (int i=0; i<numTargets-1; i++) {
        ...
        fullStats[i][1] = std::abs(refIntegral) * MeterOverlap<T>::ratioErr(targetRatioStats[i][MeterOverlap<T>::AVG_AVG], targetRatioStats[i][MeterOverlap<T>::AVG_ERR], vd, ed,0) * alpha;
    ...
    double refErrorRatio = refRatioStats[0][MeterOverlap<T>::AVG_ERR]/std::abs(refRatioStats[0][MeterOverlap<T>::AVG_AVG]);
    if (std::isnan(refErrorRatio) || refErrorRatio > 1) refErrorRatio = 1;
    double targetErrorRatio = targetRatioStats[0][MeterOverlap<T>::AVG_ERR]/std::abs(targetRatioStats[0][MeterOverlap<T>::AVG_AVG]);
    if (std::isnan(targetErrorRatio) || targetErrorRatio > 1) targetErrorRatio = 1;
    idealTargetFraction = 1.0/(1 + refErrorRatio/targetErrorRatio * std::sqrt((1-oldFrac)/oldFrac));
}
